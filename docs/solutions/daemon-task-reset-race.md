...
Worse: if you `systemctl restart` while the task is being dispatched, the shutdown kills the daemon but the agent subprocess (running as voltron-agent) keeps running until it finishes or times out.
...
Agent subprocesses (`sudo -u voltron-agent prlimit ... claude -p`) are not in the daemon's process group, so `systemctl stop` doesn't kill them.
...
sudo pkill -9 -u voltron-agent

# 3. Reset tasks in DB
sqlite3 data/voltron.db "UPDATE tasks SET status='queued', ... WHERE id IN (...)"
...
- Could add a CLI command: `voltron pause` / `voltron resume` to disable the executor loop
- Could add a `voltron retry <id>` command that stops, resets, and restarts atomically
